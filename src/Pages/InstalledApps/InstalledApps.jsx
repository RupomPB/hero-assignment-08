...
const InstalledApps = () => {
  ...
  const handleSort = (type) => {
    setSort(type);
    if (type === "Low-High") {
      const sortedBySize = [...installedList].sort(
        (a, b) =>
          convertedDownloads(a.downloads) - convertedDownloads(b.downloads)
      );
    ...
    if (type === "High-Low") {
      const sortedBySize = [...installedList].sort(
        (a, b) =>
          convertedDownloads(b.downloads) - convertedDownloads(a.downloads)
      );
      setInstalledList(sortedBySize);
    }
  };
  ...
  useEffect(() => {
    ...
    const installList = data.filter((app) =>
  ...
  const handleUninstall = (id) => {
    ...
    localStorage.setItem(
      "installList",
      JSON.stringify(uninstallApps.map((app) => app.id))
    );
  ...
  if (installedList.length === 0) {
    return (
      <p className="text-center text-gray-500 my-10 text-5xl font-semibold">
        No Apps Installed
      </p>
    );
  }
